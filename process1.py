...
def process1_page():
    ...
    if st.button("Extract All Parts"):
        ...
        if st.button("Assign Ward by House No Prefix"):
            ...
            st.write(f"Assigned {mask.sum()} voters to {ward} based on house_no prefix '{prefix}'.")
        ...
        wards = sorted([w for w in edited["old_ward"].unique() if isinstance(w, str) and w.strip()])
        for w in wards:
